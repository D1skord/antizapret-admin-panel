  ...
  >
    <div class="p-5 flex items-center justify-between gap-3">
      <div class="relative flex-1 max-w-sm">
        <svg class="absolute left-3 top-1/2 -translate-y-1/2 fill-gray-400 dark:fill-gray-500 pointer-events-none" width="18" height="18" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <input
          ...
          class="w-full h-10 rounded-lg border border-gray-200 bg-transparent py-2 pl-10 pr-4 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-white/[0.03] dark:text-white/90 dark:placeholder:text-white/30"
        />
      </div>
      <button @click="isModalOpen = true" class="px-4 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-600 whitespace-nowrap">
        Добавить клиента
      </button>
    </div>
    ...
    <div v-else-if="filteredUsers.length === 0" class="p-5 text-center text-gray-500 dark:text-gray-400">
    ...
    <Pagination
      v-if="!loading && totalClients > 0 && !searchQuery"
